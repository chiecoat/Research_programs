...
%-
function [vert_loc_error]=zoom_vertex_error(vert_mat,vert_stack,mask,foam_imi,box_size,theta_check)
...
sz_vert=numel(vert_stack(:,1,1,1));
...
for i1=1:n_verts
        ...
        for y_i=1:box_size
            if and(x_i==0,y_i==0)==1
                ...
                if or(or(row-sz_half<1,row+sz_half>num_y),or(col-sz_half<1,col+sz_half>num_x))==1
                    ...
                    energy_mat(y_i,x_i)=sum(sum((foam_mat-vert_stack(:,:,loc(3),loc(4))).^2))/(sz_vert^2);
                end
            end
        end
    end
    ...
    for t_i=-theta_check:theta_check
       ...
       %Theta is either 0 or negative so we have to add 120 to
        ...
        energy_loc=sum(sum((foam_mat-vert_stack(:,:,theta_mark,loc(4))).^2))/(sz_vert^2);
