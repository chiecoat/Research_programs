...
%     plot(xy_keep(:,1),xy_keep(:,2),'og')
    ...
    %we want to assign weights to the vertices. We will add L/2 to each
    %vertex 
    [vertex_centers_and_areas]=find_vertex_areas(imi_films,xy_keep);
%     plot(film_centers(:,3),film_centers(:,4),'ob')
%     axis equal
    
   %We make a binary image for the film skeleton
    hu_mat=vertex_centers_and_areas;
    ...
    imi_vertices_centers=zeros(L_sys_y,L_sys_x);
    %This is for the foam centroids
    for i1=1:numel(hu_mat(:,1))
        imi_vertices_centers(hu_mat(i1,2),hu_mat(i1,1))=hu_mat(i1,3);
    end
    
    avg_ell=sum(hu_mat(:,3).^2)/sum(hu_mat(:,3));
    avg_ell_cubed=sum(hu_mat(:,3).^4)/sum(hu_mat(:,3));

    for i2=1:1
        r_start=1; r_fin=1;
        if i2==1
            imi=imi_vertices_centers;
            phi=sum(sum(imi_vertices_centers))/A_sys;
            norm=phi
            L_norm=sqrt(avg_ell)   
            continue
            tag='verts_centers_binomial';
            imwrite(imi,[imiPath '\imi_' num2str(imi_nums(num_foam)) ' film_centers.png'],'png')
        end  
        if i2==2
            imi=imi_vertices_centers;
            imi(imi_vertices_centers>0)=1;
            phi=sum(sum(imi))/A_sys;
            norm=phi;
            L_norm=1;            
            tag='verts_points_binomial';
            imwrite(imi,[imiPath '\imi_' num2str(imi_nums(num_foam)) ' film_centers.png'],'png')
        end 
        %we find how many windows and hat values here
        c_rads1=logspace(log10(1E-4*sqrt(avg_ell)),log10(sqrt(avg_ell)),50);
