...
%-
% 
function [vertex_loc,imi_out]=star_watershed_struct_2d(struct_imi,walker_stack,centers,background)
...
for i1=1:num_walkers
    ...
    for i2=1:num_theta
        ...
        my_energy=numel(mask_pos(mask_pos>background));
        list_inv(i2,:)=[i2,my_energy];
    end    
    if and(cen_inv(1)>785,cen_inv(1)<840)
        if and(cen_inv(2)>600,cen_inv(2)<650)
            keyboard
        end
    end
    list_keep=list_inv(list_inv(:,2)==max(list_inv(:,2)),:);
