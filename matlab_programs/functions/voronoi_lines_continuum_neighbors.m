...
centroid_path='E:\Chieco\Hyperuniformity\HU foams\voronoi\voro_centroids';
vertex_path='E:\Chieco\Hyperuniformity\HU foams\voronoi\voro_vertices';
...
%
filename=['E:\Chieco\Hyperuniformity\HU anneal\particle positions\voro_centroids\x_y_area_N lists\poisson_anneal pointsINI_xyAN N_cells_3000 step_0.txt'];
...
%         close(figure(1))
        ...
        poiss_cells_out=zeros(numel(cells(:,1)),4);
        ...
        for i5=1:numel(cells(:,1))
            ...
            poiss_cells_out(i5,:)=[cells(i5,2:3),abs(area_tot),n_sides];
        ...
        if abs(sum(centroid_cells_out(:,3))-1)>1E-8
            ...
            centroid_info=[centroid_path '\x_y_area_N lists\' pattern ' centroid_xyAN N_cells_' cell_txt ' step_' num2str(ann) '.txt'];
            dlmwrite(centroid_info,centroid_cells_out,'Delimiter',',','Precision','%1.15e','newline','pc');
            poiss_info=[centroid_path '\x_y_area_N lists\' pattern ' pointsINI_xyAN N_cells_' cell_txt ' step_' num2str(ann) '.txt'];
            dlmwrite(poiss_info,poiss_cells_out,'Delimiter',',','Precision','%1.15e','newline','pc');
            verts_out=[new_v_pos(new_v_pos(:,3)>0,1:2),new_v_pos(new_v_pos(:,3)>0,4)];
            vert_info_tot=[vertex_path '\' pattern ' vertex_xyID N_cells_' cell_txt ' step_' num2str(ann) '.txt'];
            dlmwrite(vert_info_tot,verts_out,'Delimiter',',','Precision','%1.15e','newline','pc');
            %We want to identify the location of the next centroid
            %based on the average position of its neighbors.
            n_bubs=numel(cells_tot_out(:,1));
            vert_mat=cells_tot_out(:,8:end);
            v_sz=size(vert_mat);
            cell_cens_new=zeros(n_bubs,2);
            for i1=1:n_bubs
                id=cells_tot_out(i1,1);
                verts=vert_mat(i1,vert_mat(i1,:)>0);
                n_verts=numel(verts);
                bub_id_mat=zeros(n_verts,3)-1;
                for i2=1:n_verts
                    vert_index=find(vert_mat==verts(i2));
                    [row,col]=ind2sub(v_sz,vert_index);
                    bub_id_mat(i2,1:numel(row))=row';
                end
                bub_id_vec=reshape(bub_id_mat,[1,3*n_verts]);
                bub_tot_ids=unique(bub_id_vec(bub_id_vec>0));
                %The new center will be definied by the average
                %position of the centers of the neighboring cells.
                %However if there is only 1 neighboring cell then the
                %current cell will be its own neighbor and the next
                %cell will be the the midpoint. this can *only* happen
                %at the corner
                if numel(bub_tot_ids)==2
                    bub_neighbors=bub_tot_ids;                    
                else
                    bub_neighbors=bub_tot_ids(bub_tot_ids~=id);
                end
                n_neighbors=numel(bub_neighbors);
                cell_cens_new(i1,:)=[sum(cells_tot_out(bub_neighbors,4)),sum(cells_tot_out(bub_neighbors,5))]/n_neighbors;
            end
            cen_neighbors_info=[centroid_path '\x_y_area_N lists\' pattern ' cenneighbors_xyAN N_cells_' cell_txt ' step_' num2str(ann) '.txt'];
            dlmwrite(cen_neighbors_info,[cell_cens_new,centroid_cells_out(:,3:4)],'Delimiter',',','Precision','%1.15e','newline','pc');
            cell_info_tot=[centroid_path '\all info list\' pattern ' ID_NA_xyp_xyc_xyneigh_vertID N_cells_' cell_txt ' step_' num2str(ann) '.txt'];            
            dlmwrite(cell_info_tot,[cells_tot_out(:,1:7),cell_cens_new,cells_tot_out(:,8:end)],'Delimiter',',','Precision','%1.15e','newline','pc');
        end
    end
end
