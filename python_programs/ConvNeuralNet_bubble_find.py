# -*- coding: utf-8 -*-
"""
Created on Tue Oct 18 11:57:34 2022
...
image_paths_pandas=pd.read_csv("E:/Chieco/NASA foam/Images foam/ttab_25%/machine learning lists/image_post_its/train_foam_ttab25_im_names_list_bubbles_all_ages.csv")
image_paths=image_paths_pandas['post_it_names']

#read in trainin data, whether regions are or are not bubbles
bub_data=pd.read_csv("E:/Chieco/NASA foam/Images foam/ttab_25%/machine learning lists/image_post_its/train_foam_ttab25_bubble_data_all_ages.csv")
...
y = np.reshape(pd.Series.to_numpy(y), (-1,1))


"""
# List image categories we are interested in
LABELS = set(["dalmatian", "hedgehog", "llama", "panda"])
This is from the example code but may help us understand later
...
image_stack = np.array(image_stack, dtype="float") / 255.0

# Show data shape
print("Shape of whole data: ", image_stack.shape)
...
X_train, X_test, y_train, y_test = train_test_split(image_stack, y, test_size=0.2, random_state=0)
#we need these to be explicitly one column vectors
